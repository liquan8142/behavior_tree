...
行为树（Behavior Trees）与状态机类似，本质上是一种在正确时间和条件下调用__回调函数__的机制。
这些回调函数内部发生什么完全由你决定。

我们将交替使用__"调用回调函数"__和__"触发"__这两个表达方式。
...
或者，我们可以使用__依赖注入__来根据函数指针（即"函数对象"）创建树节点。
...
让我们考虑名为__my_tree.xml__的以下XML文件：
