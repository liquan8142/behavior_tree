...
尽管库是用C++编写的，但树本身可以在 _运行时_ 创建和组合，更具体地说，在 _部署时_ 使用基于XML的脚本语言。
...
- 节点的端口数量、名称和类型必须在 _编译时_ （C++）已知；端口之间的连接在 _部署时_ （XML）完成。

- 你可以存储任何C++类型作为值（我们使用类似于[std::any](https://www.fluentcpp.com/2021/02/05/how-stdany-works/)的 _类型擦除_ 技术）。